...
export function getAllPosts() {
  const postsFilePath = fs.readdirSync(POST_PATH).filter(path => /\.mdx?$/.test(path));
...
export function getPostBySlug(slug) {
  ...
  const { content, data } = matter(fs.readFileSync(path.join(POST_PATH, `${post.filePath}`), 'utf8'));
  return {
    content,
    data
  }
